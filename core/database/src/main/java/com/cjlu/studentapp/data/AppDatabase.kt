...
interface StudentRequestDao {
    ...
    fun observeForStudent(studentId: String): Flow<List<StudentRequestEntity>>

    @Query("SELECT * FROM student_requests WHERE studentId = :studentId ORDER BY createdAtMillis DESC")
    suspend fun getRequestsForStudent(studentId: String): List<StudentRequestEntity>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(requests: List<StudentRequestEntity>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(request: StudentRequestEntity)
